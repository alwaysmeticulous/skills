...
**Purpose:** Record an agent's verdict on one screenshot difference, backed by a review comment containing a succinct reason at required approximate normalized coordinates. Returns the created comment (including its `id`) — reuse that `id` for a later `reply-to-diff-comment` (e.g. to reply "Fixed" once addressed) instead of looking it up again via `agent diff-comments`. The two are **not symmetric**:
